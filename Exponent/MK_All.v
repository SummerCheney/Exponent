...
(* 定义86 *)
Definition FirstMember z r x := z ∈ x
  /\ (∀ y, y ∈ x -> ~ Rrelation y r z).

(* 定义87 *)
Definition WellOrdered r x := Connect r x
  /\ (∀ y, y ⊂ x -> y ≠ Φ -> ∃ z, FirstMember z r y).
...
(* 定义89 *)
Definition Section y r x := y ⊂ x /\ WellOrdered r x
  /\ (∀ u v, u ∈ x -> v ∈ y -> Rrelation u r v -> u ∈ y).

(* 定理90 *)
Theorem MKT90 : ∀ n x r, n ≠ Φ -> (∀ y, y ∈ n -> Section y r x)
...
(* 定理91 *)
Theorem MKT91 : ∀ {x y r}, Section y r x ->  y <> x
...
(* 定理92 *)
Theorem MKT92 : ∀ {x y z r}, Section x r z -> Section y r z
  -> x ⊂ y \/ y ⊂ x.
Proof.
  intros. TF (x ⊂ y); auto.
...
(* 定义93 *)
Definition Order_Pr f r s := Function f /\ WellOrdered r dom(f)
  /\ WellOrdered s ran(f) /\ (∀ u v, u ∈ dom(f) -> v ∈ dom(f)
    -> Rrelation u r v -> Rrelation f[u] s f[v]).

(* 定理94 *)
Theorem MKT94 : ∀ {x r y f}, Section x r y -> Order_Pr f r r
...
Lemma lem97a :  ∀ f g u r s x y, Order_Pr f r s -> Order_Pr g r s
  ...
  -> Section ran(f) s y -> Section ran(g) s y
...
(* 定义98 *)
Definition Order_PXY f x y r s := WellOrdered r x
  /\ WellOrdered s y /\ Order_Pr f r s
  /\ Section dom(f) r x /\ Section ran(f) s y.

(* 定理99 *)
Lemma Lemma99c : ∀ y r x a b, Section y r x -> a ∈ y -> ~ b ∈ y
  -> b ∈ x -> Rrelation a r b.
Proof.
...
Proof.
  ...
  { apply H2; red; intros.
    ...
    - intro. apply H5. destruct H2. eqext; auto. Absurd.
      feine z. rewrite <-H7. auto. }
  assert (∃ u, FirstMember u s (y ~ ran(f))).
  { apply H3; red; intros.
    - apply AxiomII in H8; tauto.
    - intro. apply H6. destruct H3. eqext; auto. Absurd.
  ...
  deandG; auto; try red; deandG; auto; try red; deandG; auto.
  - eapply wosub; eauto.
  - eapply wosub; eauto.
...
(* 正则公理 VII *)
Axiom AxiomVII : ∀ x, x ≠ Φ -> ∃ y, y ∈ x /\ x ∩ y = Φ.
...
(* 定理114 *)
Theorem MKT114 : ∀ x, Section x E R -> Ordinal x.
Proof.
...
Corollary Property114 : ∀ x, Ordinal x -> Section x E R.
Proof.
...
Proof.
  ...
  - assert (∃ u, FirstMember u E \{λ a, a ∈ dom(f) ∩ dom(h)
    ...
    { apply (MKT107 MKT113a); red; intros.
      ...
      - intro. apply H5; intros. Absurd. feine a.
        rewrite <-H6; appA2G. }
...
Proof.
  ...
  { apply MKT114; unfold Section; deandG; intros.
  ...
  - assert (∃ y, FirstMember y E (R ~ dom(f))).
    ...
    { appA2H H8. deor; auto. eins H9. appoA2G. split; auto.
      ...
      { apply MKT114; unfold Section,h; deandG; intros.
...
(* 无限性公理 VIII *)
Axiom AxiomVIII : ∃ y, Ensemble y /\ Φ ∈ y
  /\ (∀ x, x ∈ y -> (x ∪ [x]) ∈ y).
...
Proof.
  ...
  { apply (@ MKT107 ω); auto; red; intros.
    - appA2H H3; tauto.
    - intro. apply H2. eqext; auto. Absurd.
...
(* 定义139 *)
Definition ChoiceFunction c := Function c
  /\ (∀ x, x ∈ dom(c) -> c[x] ∈ x).
...
Definition Equivalent x y := ∃ f, Function1_1 f
  /\ dom(f) = x /\ ran(f) = y.
...
Definition Cardinal_Number x  := Ordinal_Number x
  /\ (∀ y, y ∈ R -> y ≺ x -> ~ (x ≈ y)).
